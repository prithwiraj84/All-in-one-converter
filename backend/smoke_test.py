...
def docx_bytes() -> bytes:
    from docx import Document  # provided transitively by pdf2docx

    doc = Document()
    doc.add_heading("All in one converter smoke test", 0)
    doc.add_paragraph("Word to PDF round-trip. Hello world 12345.")
    buf = io.BytesIO()
    doc.save(buf)
    return buf.getvalue()
...
def pptx_bytes() -> bytes | None:
    """A real .pptx — requires python-pptx (optional, not a runtime dep)."""
    try:
    ...
    slide.shapes.title.text = "PPT to PDF smoke test"
    buf = io.BytesIO()
    prs.save(buf)
    return buf.getvalue()


def mp4_bytes() -> bytes | None:
    """A 1-second test clip generated by FFmpeg; None if FFmpeg is missing."""
    import shutil
    import subprocess
    import tempfile
    ...
    ffmpeg = shutil.which(settings.ffmpeg_cmd)
    if not ffmpeg:
        return None
...
def main() -> None:
    ...
    # ── Document ───────────────────────────────────────────────────
    # PDF->Excel uses pdfplumber; PDF->Word uses pdf2docx; the *->PDF
    # directions use LibreOffice (soffice).
    run("pdf-to-excel", "/api/document/pdf-to-excel", [pdf_file()])
    run("pdf-to-word", "/api/document/pdf-to-word", [pdf_file()])
    run("word-to-pdf", "/api/document/word-to-pdf",
        [("files", ("doc.docx", docx_bytes(),
                    "application/vnd.openxmlformats-officedocument.wordprocessingml.document"))])
    run("excel-to-pdf", "/api/document/excel-to-pdf",
        [("files", ("sheet.xlsx", xlsx_bytes(),
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"))])
    pptx = pptx_bytes()
    if pptx is not None:
        run("ppt-to-pdf", "/api/document/ppt-to-pdf",
            [("files", ("deck.pptx", pptx,
                        "application/vnd.openxmlformats-officedocument.presentationml.presentation"))])
    else:
        RESULTS.append(("ppt-to-pdf", "SKIP", "install python-pptx to test this tool"))
    ...
    mp4 = mp4_bytes()
    if mp4 is not None:
        run("video-converter", "/api/video/convert",
            [("files", ("clip.mp4", mp4, "video/mp4"))], {"target": "webm"})
    else:
        RESULTS.append(("video-converter", "NEEDS BINARY", "FFmpeg not available to generate/convert a test clip"))
