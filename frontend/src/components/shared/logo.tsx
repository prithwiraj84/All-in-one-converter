...
export function Logo({ className, showText = true }: { className?: string; showText?: boolean }) {
  return (
    <Link href="/" className={cn("group flex items-center gap-2.5", className)} aria-label="All in one convertor home">
      ...
      {showText && (
        <span className="text-[15px] font-semibold leading-none tracking-tight">
          All in one<span className="text-gradient"> convertor</span>
        </span>
      )}
    </Link>
  );
